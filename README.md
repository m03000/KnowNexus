...
本地优先的 Windows AI 知识工作台，当前版本仅为初步设计版本，如有问题，请见谅。
...
2. 双击根目录中的 `KnowNexus.exe`，也可以使用带相同图标的 `Start KnowNexus.exe` 快捷启动器。
...
你可以将抖音、b站等第三方平台的链接交给它，从而生成 AI 笔记，这些笔记胡自动进入 LLM Wiki 维护成您的知识体系。
...
内置 Codex 配置会保留，但首次运行默认关闭，不会未经用户操作自动监听。
...
可以导入和阅读 Markdown、TXT、HTML、Office 文档、PDF、图片、音频和视频等资料。

- 图片与扫描 PDF 使用随包提供的 Tesseract OCR。
- 音频和视频使用随包提供的 Whisper small 进行转写。
- 文件越大，处理时间越长；长视频建议先裁剪需要处理的部分。
...
```text
C:\Users\你的用户名\AppData\Roaming\KnowNexus
```
...
删除程序文件夹不会自动删除 `%APPDATA%\KnowNexus`。清理或迁移前请先备份需要保留的数据。
...
5. 查看 `%APPDATA%\KnowNexus\logs` 中的日志。

### 关闭窗口后程序仍在运行

这是系统托盘功能。需要完全退出时，请右键托盘图标并选择“退出 KnowNexus”。

### 本地检索不可用

进入“设置 → 基础配置”，确认 Embedding 和 Reranker 都显示“已就绪”。模型下载需要网络连接和足够的磁盘空间。
