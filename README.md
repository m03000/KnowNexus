...
KnowNexus的所有功能都被拆分到了主页各个组成物品。  
点击主页左下角设置进行基础配置，首次进入需要下载必要模型和多模态组件（如需要）。  
点击左侧书籍可以进入笔记页面，点击窗户则可以进入知识图谱界面。  
鼠标触碰右侧日常清单会显示记忆相关信息，点击则进入仪表盘界面可以查看所有内容。
如想开启项目分析可以在设置中打开，之后可以点击电脑界面进入。
...
若需要llm识别平台链接、图片等内容则需要完整下载releases里面打包好的桌面端应用或者在应用内下载。  
在下载ocr启动器时需要将下载地址选择为设置里面的推荐地址。
